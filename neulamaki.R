...
## Data and map crop ----
# fils <- list.files("data/neulamaki/", full.names = T)

r1 <- raster("data/neulamaki/P5114A.tif")
r2 <- raster("data/neulamaki/P5114B.tif")
r3 <- raster("data/neulamaki/P5114C.tif")
r4 <- raster("data/neulamaki/P5114D.tif")
rr1 <- merge(r1,r2)
rr2 <- merge(r3,r4)
rr <- merge(rr1,rr2)
rm(list = c("r1","r2", "r3", "r4","rr1","rr2"))
...
## Map crop
# 62.847193915121615, 27.55824594866513
# 62.91750670779432, 27.62738355504846
long_range   = c(27.55824594866513, 27.62738355504846)
lat_range = c(62.847193915121615, 62.91750670779432)
...
base_map = zoom_mat %>% 
  ...
  # add_overlay(generate_scalebar_overlay(
  #   extent = attr(rr, "extent"), length = 50000,
  #   heightmap = rr, 
  #   latlong=TRUE))
...
base_map %>% 
  ...
  #                                   color="red",
  #                                   heightmap = zoom_mat)) %>% 
  plot_map(title_text = "Neulamäki, Kuopio. Data: Maanmittauslaitos Maastotietokanta 03/2022 & OpenStreetMap.org", 
           title_size = 20,  
           title_offset = c(15,15),
           title_bar_color = "grey5", 
           title_color = "white", 
           title_bar_alpha = 1) 
...
#### To Movie ----
if(interactive()) {
  filename_movie = "img/trailmaps/neulamaki.mp4" #tempfile()
